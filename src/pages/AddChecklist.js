...
const Createchecklist = () => {
  ...
  const handleSubmit = async () => {
    ...
    newStore.append('type', 'checklist');
    ...
    if (storeResult?.success) {
      ...
      successToast(isEditedData ? 'checklist Updated Successfully' : 'checklist Added Successfully');
    } else {
  ...
  const doFetchAllBlogData = async () => {
    try {
      ...
      const bearerToken = Cookies.get('x-access-token')
        ...
        : null;
      // debugger;
      const { data } = await axios.get(`${BASE_URL}content/admin/list?type=checklist`, {
        headers: {
          authorization: `Bearer ${bearerToken}`,
        },
      });
      ...
      {fetchLoading ? (
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <Card sx={{ py: 2, px: 3 }}>
              <h1>{isEditedData ? 'Edit checklist' : ' Add checklist'} </h1>
              <Grid item xs={12} md={12}>
                <TextField
                ...
                >
                  {isLoading ? (
                    ...
                    >
                      {isEditedData ? 'Edit checklist' : 'Create checklist'}
                    </LoadingButton>
                  )}
                </div>
              </Grid>

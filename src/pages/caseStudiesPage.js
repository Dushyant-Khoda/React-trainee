...
export default function CategoriesPage() {
  ...
  const fetchEbook = async () => {
    try {
      ...
      const { data } = await axios.get(`${BASE_URL}content/admin/upload/list?type=casestudies`, {
        headers: {
  ...
  const addContent = async (contentData) => {
    try {
      ...
      contentFormData.append('type', 'casestudies');
        ...
        <>
          <Container>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
              ...
              <Link to={'/dashboard/casestudies/add'}>
                <Button
                  variant="contained"
                  onClick={() => {
                    // seteditcatTitle('');
          ...
          >
            <MenuItem
              onClick={() => {
                handleCloseMenu();
                navigate(`/dashboard/casestudies/edit/${UpdateData._id}`);
                // setUpdateModalopen(true);

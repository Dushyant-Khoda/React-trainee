  ...
  const doFetchAllBlogData = async () => {
    try {
      ...
      const bearerToken = Cookies.get('x-access-token')
        ...
        : null;
      // debugger;
      const { data } = await axios.get(`${BASE_URL}content/admin/list?type=infographics`, {
        headers: {
          authorization: `Bearer ${bearerToken}`,
        },
      });
      ...
      {fetchLoading ? (
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <Card sx={{ py: 2, px: 3 }}>
              <h1>{isEditedData ? 'Edit infographics' : ' Add infographics'} </h1>
              <Grid item xs={12} md={12}>
                <TextField
                ...
                >
                  {isLoading ? (
                    ...
                    >
                      {isEditedData ? 'Edit infographics' : 'Create infographics'}
                    </LoadingButton>
                  )}
                </div>
              </Grid>
